...
classdef bodyMesh < BODY
    ...
    methods
        ...
        function obj = bodyMesh(verts, faces) 
            obj.body_type = 'mesh'; 
            obj.verts = verts;      obj.num_verts = max(size(obj.verts));
            obj.faces = faces;      obj.num_faces = max(size(obj.faces));
            
            obj.verts_local = zeros(obj.num_verts,3);
            obj.verts_world = zeros(obj.num_verts,3);
            for v=1:obj.num_verts
               obj.verts_local(v,1) = obj.verts(v).local_coords(1); % TODO: use this instead of verts
               obj.verts_local(v,2) = obj.verts(v).local_coords(2);
               obj.verts_local(v,3) = obj.verts(v).local_coords(3);
               obj.verts_world(v,1) = obj.verts(v).world_coords(1);
               obj.verts_world(v,2) = obj.verts(v).world_coords(2);
               obj.verts_world(v,3) = obj.verts(v).world_coords(3);
            end
            ...
            for e = 1:obj.num_edges
                ...
                E = obj.edges(e); 
                V1 = obj.verts(v1).local_coords;
                V2 = obj.verts(v2).local_coords; 
                ...
                for fv=1:f.num_verts
                    if f.verts(fv) == v1
                        if f.verts(mod(fv,f.num_verts)+1) == v2 
                            ...
                            E.t1 = cross3( f.normal, V2-V1 );   
                            E.t1 = E.t1/norm(E.t1);
                            E.t2 = cross3( V2-V1, obj.faces(E.faces(2)).normal ); 
                            E.t2 = E.t2/norm(E.t2); 
                            break;
                        end
                    elseif f.verts(fv) == v2
                        if f.verts(mod(fv,f.num_verts)+1) == v1 
                            ...
                            E.t1 = cross3( V2-V1, f.normal );   
                            E.t1 = E.t1/norm(E.t1);
                            E.t2 = cross3( obj.faces(E.faces(2)).normal, V2-V1 ); 
                            E.t2 = E.t2/norm(E.t2); 
                            break;
                        end
                    end
        ...
        function obj = update_world_position( obj )
            ...
            for i = 1:obj.num_verts
                obj.verts(i).world_coords = obj.u + qtrotate(obj.quat, obj.verts(i).local_coords);
            end
        ...
        function draw( obj )
            ...
            elseif ~obj.static  % Don't redraw static objects.
                % Transform data to world frame coordinates
                X = zeros(size(obj.Xdata));
                Y = zeros(size(obj.Ydata));
                Z = zeros(size(obj.Zdata));
