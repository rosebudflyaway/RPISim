            ...
            if (norm(v1-B2.u) - B2.bound) < 0   % Check if vertex is within the bounding sphere of b2
               ...
               if psi > epsilon_minus && psi < epsilon_plus
                   ...
                   if triDist < epsilon_plus   % triDist is always positive
                       ...
                       if psi > max_psi
                          max_psi = psi; % Track largest psi 
                       end
                       % PEG heuristic
                       if (psi >= -10^-6 && psi < min_psi) || (psi == max_psi && psi < -10^-6)    % There is a new non-zero-ish value here 
                           if psi >= -10^-6 && psi < min_psi
                                min_psi = psi;
                           end
...
%         end
        ...
        %if psi > epsilon_minus && psi < epsilon_plus    
        if psi < 0.2    % TODO: Use a small but reasonable eps for this particular check
