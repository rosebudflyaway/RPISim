...
function inPenetration = doEdgesPenetrate(B1,B2,e1,e2)
    ...
    for f = 1:B2.num_faces
        ...
        if does_edge_intersect_face( e1a, e1ab, ...
                                     B2.verts(B2.faces(B2.edges(f).faces(1)).verts(1)).world_coords', ...
                                     B2.verts(B2.faces(B2.edges(f).faces(1)).verts(2)).world_coords', ...
                                     B2.verts(B2.faces(B2.edges(f).faces(1)).verts(3)).world_coords' )
           e1xB2 = e1xB2 + 1;
           if e1xB2 > 1 
               break;       % At this point, e1 penetrates two faces
           end
        end
    end % End e1 loop  
    if e1xB2 < 2
       return;      % We require that e1 penetrates at least two faces
    end
    
    % Check remaining faces of B1 if necessary
    for f = 1:B1.num_faces
       if f == B1f1 || f == B1f2
           continue;    % Avoid redundantly counting faces of e1
       end
       if does_edge_intersect_face( e2a, e2ab, ...
                                    B1.verts(B1.faces(B1.edges(f).faces(1)).verts(1)).world_coords', ...
                                    B1.verts(B1.faces(B1.edges(f).faces(1)).verts(2)).world_coords', ...
                                    B1.verts(B1.faces(B1.edges(f).faces(1)).verts(3)).world_coords' )
            e2xB1 = e2xB1+1;
            if e2xB1 > 1
               inPenetration = true;
               return; 
