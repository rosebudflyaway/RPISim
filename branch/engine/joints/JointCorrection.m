...
function JointCorrection( sim )
  ...
  %% RECORD ERROR
  sim.jointError.Perr(sim.STEP) = 0;
  sim.jointError.Verr(sim.STEP) = 0;
  for j=1:nj
     J = sim.Joints(j);
     [~, C, Cdot] = J.getJointCorrectionValues();
     sim.jointError.Perr(sim.STEP) = sim.jointError.Perr(sim.STEP) + norm(C);
     sim.jointError.Verr(sim.STEP) = sim.jointError.Verr(sim.STEP) + norm(Cdot); 
  end
