...
classdef Simulation < handle
   properties
      ...
      hController         % Handle to any user-defined controllers for force/torque generation
      ...
      useBULLET           % Boolean whether to use Bullet collision detection
      
      % Joint variables
      Joints              % The set of body joints
      jointCorrection     % Boolean, true -> use joint correction 
      num_jointConstraints
      num_activeJointBodies
      jointError          % Struct that records every timestep
   ...
   methods
      ...
      function obj = Simulation(P,h)
          ...
          obj.FORMULATION = 'mLCP'; % Default formluation & solver
          obj.hFormulation = @mLCPdynamics;
          obj.SOLVER = 'PATH';  
          obj.hSolver = @pathsolver; 
          obj.num_fricdirs = 7; 
          obj.FRICTION = true; 
          obj.GRAVITY = true;
          obj.COLLISIONS = true;
          ...
          obj.DATA_DIRECTORY = ['sim_data_' datestr(now,'yy_dd_HH_MM_SS')]; 
      ...
      function obj = step(obj)
        ...
        if ~isempty(obj.Contacts) || ~isempty(obj.Joints)
           ...
           tic;
             feval(obj.hSolver, obj);           % Solve dynamics
        ...
        if obj.COLLECT_DATA && ~isempty(obj.Contacts), sim_CollectData(obj); end
      ...
      function obj = addJoint(obj, body1, body2, pos, Zaxis, type)
          ...
          q = [ 1+dot([0;0;1],Zaxis); cross3([0;0;1], Zaxis) ];
          q = q/norm(q);                                        % Rotation to joint frame from world frame

          pos1 = qt2rot(qtinv(body1.quat)) * (pos-body1.u); % Joint position calculated by body1 (world frame)
          pos2 = qt2rot(qtinv(body2.quat)) * (pos-body2.u); % Joint position calculated by body2 (world frame)
          ...
          obj.Joints(end).constraintIndex = ...
              obj.num_jointConstraints+1 : obj.num_jointConstraints + obj.Joints(end).numConstraints;
          obj.num_jointConstraints = obj.num_jointConstraints + obj.Joints(end).numConstraints; 
      ...
      function obj = solver(obj, solver)
        % Solver handle
        if     strcmp(solver,'PATH'),              obj.hSolver = @pathsolver; 
        elseif strcmp(solver,'Lemke'),             obj.hSolver = @LemkeWrapper;       
        elseif strcmp(solver,'fischer_newton'),    obj.hSolver = @fischer_newton_capsim; 
        elseif strcmp(solver,'penalty'),           obj.hSolver = @penalty;
        elseif strcmp(solver,'mlcp_fixed_point'),  obj.hSolver = @mlcp_fixed_point;
        elseif strcmp(solver,'mncp_fixed_point')  
            obj.hSolver = @mncp_fixed_point;
            obj.num_fricdirs = 2;
        elseif strcmp(solver,'mncp_fixed_point_pgs')    
            obj.hSolver = @mncp_fixed_point_pgs;
            obj.num_fricdirs = 2;
      ...
      function obj = run(obj, varargin)
        ...
        if obj.useBULLET
            obj.hCollisionDetection = @BulletCD; 
        elseif strcmp(obj.FORMULATION,'PEG')    % TODO: Only PEG uses alternative CD at present, but I think this will need better handling.  
      ...
      %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
      %% controller()
      % Sets a controller to generate forces/torques in preDynamics.
      % controllerFunctionHandle is a string of the controller function name. 
      function obj = controller( obj, controllerFunctionHandle )
          obj.hController = controllerFunctionHandle; 
      end
      ...
      function obj = gravityON(obj)
          for i=1:length(obj.P)
              if ~obj.P{i}.static
                ...
                obj.P{i}.Aext(3) = obj.P{i}.Aext(3) -9.81;  % Update acceleration
                obj.P{i}.Fext = obj.P{i}.mass * obj.P{i}.Aext;
              end
          end
      end
