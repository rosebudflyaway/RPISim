...
function sim_draw_collisions( obj )
    ...
    %figure(obj.GUI.SIM_FIG);   % This is good to do, but makes the GUI flicker :(
    if strcmp(obj.FORMULATION,'PEG')
        hi=0; % handle index
        ...
        for c=1:length(C)
            ...
            for sc=1:size(C(c).p2,2)
               fprintf('%f, ', C(c).psi_n(sc)); 
            end
            if max(C(c).psi_n) < -.0001
               fprintf('  *******************************   <----- PENETRATION??? \n'); 
            end
            fprintf('\n'); 
...
%                 disp(['e-e (' num2str(c) ') : ' num2str(C(c).psi_n) ', ' num2str(C(c).normal')]);
        ...
        for c=1:length(C)
            for sc = 1:size(C(c).p2,2)
                ...
                obj.GUI.Hg(hi) = plot3([p1(1) p2(1)],[p1(2) p2(2)],[p1(3) p2(3)],'r');
                obj.GUI.Hg(hi+1) = plot3(p1(1),p1(2),p1(3),'go');
                obj.GUI.Hg(hi+2) = plot3(p2(1),p2(2),p2(3),'gx');
                hi = hi+2;
            end
        end
    end
