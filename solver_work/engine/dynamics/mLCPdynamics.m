...
function mLCPdynamics( sim )
  ...
  NU = sim.bodies.velocities;           % Vector of velocities, including rotational
  FX = sim.bodies.forces;           % Vector of external forces, including rotational
  PSI = sim.contacts.PSI;         % Vector of gap distances 
  ...
  sim.dynamics.A_MCP = A;
  sim.dynamics.b_MCP = b;
  sim.dynamics.z0_MCP = zeros(length(b),1); 
end 
