...
function preDynamics( sim )
  ...
  for j=1:nj
     ...
     if ~Jnt.body1.static
        Gb(6*b1id-5:6*b1id, 5*j-4:5*j) = G1c; 
        if sim.FRICTION
            Gf(6*b1id-5:6*b1id, nd*nc+j) = G1f;   % TODO: not yet implemented
        end
     end
     if ~Jnt.body2.static
        Gb(6*b2id-5:6*b2id, 5*j-4:5*j) = G2c;  % TODO: fix indices 
        if sim.FRICTION
  ...
  sim.contacts.PSI = PSI; 
  
  % for bilateral constraints 
  sim.dynamics.Gb = [];
  sim.dynamics.Jacobian = [];
  sim.dynamics.Bounds = [];
  sim.dynamics.Rows = [];
  sim.dynamics.TimeDeri = [];
  sim.dynamics.Violation = [];
  
  sim.bodies.velocities = NU;
  sim.bodies.masses = M;
  sim.bodies.forces = FX;
  sim.constraints.pairs = [];
  sim.dynamics.h = sim.h;
  sim.dynamics.nd = nd;
  %if nd ~= 2
  %    [sim.dynamics.A_LCP sim.dynamics.b_LCP sim.dynamics.z0_LCP] = makeLCP(sim);
  %    [sim.dynamics.A_MCP sim.dynamics.b_MCP sim.dynamics.z0_MCP] = makeMCP(sim);
  % end
end
