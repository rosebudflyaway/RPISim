...
strframe = strcat('frame', num2str(obj.FrameCounter));
%% bodies
...
NTB  =  length(obj.P);    % number of total bodies 
...
for i = 1 : NB
    ...
    NU(6*i-5:6*i, 1) = obj.P{i+STA}.nu;      % velocities: 6*n by 1
    FORCE(6*i-5:6*i, 1) = obj.P{i+STA}.Fext; % forces: 6*n by 1
    MASS(i, 1) = obj.P{i+STA}.mass;          % masses:  n by 1
    INERTIA(3*i-2:3*i, :) = obj.P{i+STA}.J;  % Inertia_tensor: 3*n by 3
end
POSITION = POSITION';             % 3 by n
QUATERNION =  QUATERNION';       % 4 by n
NU = NU';                         % 1 by n
FORCE = FORCE';                   % 1 by 6*n
MASS = MASS';                     % 1 by n
INERTIA = INERTIA';              % 3 by 3*n
...
obj.data_set.(strframe).bodies.inertia = INERTIA;
...
GAP  = zeros(NC, 1);  % NC by 1
MU_S = [];            % static coefficient of friction
MU_K = zeros(NC, 1);  % kinematic coefficient of friction  % NC by 1
PAIRS = zeros(NC, 2); % NC by 2
NORMAL = zeros(NC, 3);% NC by 3
POINTS = zeros(NC, 3);% NC by 3
...
%% Settings 
obj.data_set.(strframe).settings.step_size = obj.h;  
end
