...
function results = compareSolvers(solverHandles, problemFile, ErrorParams, problem_indicies, TuneParams, source)
            ...
            for i=1:length(solverHandles)
                ...
                if length(s) >= 4 && strcmp(s(1:4),'wrap')
                    ...
                    elseif strcmpi(solverHandles{i},'PGS')
                        SOLVER_HANDLES{i} = @wrap_LcpPGS;
                    elseif strcmpi(solverHandles{i},'mpgs')
