...
%     if rcond(full(J)) < 2*eps
%         p = pinv(J)*Fk;
%     else
%         p = -J\Fk;
%     end  
    [p ~]  = gmres( J, -Fk, restart);
     %p  = - J\Fk;
    dx = p(1:N);
    dy = p(N+1:end);
...
while iter <= max_iter,
    ...
    sigma =  1 ./ iter;            % Goes tmo zero as we iterate
