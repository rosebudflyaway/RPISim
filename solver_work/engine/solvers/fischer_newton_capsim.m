function [x,iter] = fischer_newton_capsim(M,q,l,u,x0,display)
...
for iter = 1:max_iter
    ...
    if display == true
        disp('iter = %2d, psi = %3.0e, r = %3.1f, mu = %3.0e',iter,psi,r,mu);
    end
...
if display == 2 && iter == max_iter
    fprintf('Maximum iterations = %2d reached, rms(dx) = %3.0e, r = %3.1f, mu = %3.0e',iter,sqrt(mean(dx.^2)),r,mu);
end
