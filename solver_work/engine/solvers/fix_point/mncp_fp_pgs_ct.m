function sim  = mncp_fp_pgs_ct(sim)
% This is contact point wise; take every contact(both normal and frictional) as a block
% For each block, we will get pn, pf_t pf_o as solution to a certain contact
% To solve each block, we could use any kind of solver we want.
M = sim.dynamics.M;
Gn = sim.dynamics.Gn;
Gf = sim.dynamics.Gf;
h = sim.h;
